...
import AgentDB from "@external/database/entities/agent"
...
export default class AgentRepository implements IAgentRepository {
  ...
  async getAgent(
    ...
    limit: string| undefined): Promise<IAgent[] | null> {
      ...
      const ret: IAgent[] | null = await AgentDB.find(
  ...
  async createAgent(data: IAgent[]): Promise<IAgent[]> {
    const ret: IAgent[] = await AgentDB.create(data)
    return ret
  }
  ...
  async updateAgent(id: string, data: IAgent): Promise<IAgent | null> {
    const ret: IAgent | null = await AgentDB.findByIdAndUpdate(id, data)
    return ret
  }
  ...
  async deleteAgent(id: string): Promise<IAgent | null> {
    const ret: IAgent | null = await AgentDB.findByIdAndDelete(id)
    return ret
  }
}
